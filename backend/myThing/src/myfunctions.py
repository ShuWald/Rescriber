# My additional functions, especially focused on helping working.py in logging, debugging, and more
...
# Will need adjustments based on different agent responses formats
def _extract_agent_text(agent_response):
...
# Invokes a langchain agent 
def _invoke_agent(agent, agent_name, message):
    try:
    ...
    agent_text = _extract_agent_text(raw)
    print(f"[AGENT:{agent_name}] {agent_text}")
    log_to_file(f"[AGENT:{agent_name}] Response: {agent_text}")
...
def initialize_agent_handlers(app, all_agents):
    ...
    for agent_name, agent in all_agents.items():
        def _handler(msg, name=agent_name, agent_obj=agent):
            return _invoke_agent(agent_obj, name, msg)
...
# Tests core backend functions
def test_functions():
    from flask import Flask
    
    # Create isolated test app so main app isn't locked after requests
